...
3. **GROUP BY department** -> данные сгруппированы по отделам.
...
6. **ORDER BY employee_count DESC** -> данные отсортированы по убыванию количества сотрудников.
7. **LIMIT 3** -> выбраны только первые 3 записи.
...
| Фильтрация агрегатов через `WHERE`, а не через `HAVING` | `WHERE` применяется до `GROUP BY`             |
