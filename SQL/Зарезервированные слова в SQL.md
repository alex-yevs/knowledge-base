...
✅ Работающий способ в случае необходимости такого наименования - **экранировать** идентификатор:
...
4. Используй авто дополнение в IDE - оно помогает избежать конфликтов.
...
- Ключевые слова в именах (`SELECT`, `ORDER`, `TABLE` и т.д.);
- Имена с пробелами (`user name`);
- Имена, содержащие спецсимволы (`total$amount`);
- Регистр зависимые имена (в некоторых СУБД).
